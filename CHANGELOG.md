...
- **1.10.0** - Show an offline unit as unreachable instead of guessing; quieter logs (September 2026)
  - Added: **a unit whose Wi-Fi adapter is offline now shows "No Response" in HomeKit** instead of a confident, wrong tile. When an adapter drops off the network the Kumo cloud keeps serving its last known state from a frozen shadow record, so the plugin had no idea it was reading history. Observed live 2026-09-10: the rear bedroom's adapter was off the LAN for ~22h while its zone record still read `mode=cool power=1 connected=true` with an `updatedAt` from two minutes earlier — the unit was physically **off**, and an "AC off" automation sent three `off` commands that each returned HTTP 200 and reached nothing. Note that neither `adapter.connected` nor `adapter.updatedAt` in the zones payload is a freshness signal: the row is touched by unrelated refreshes, so both look healthy on a dead unit. The only reliable freshness field is `lastUpdated` on `GET /devices/{serial}/status`
  - The cloud had been reporting the truth all along via the `device_status_v2` streaming event ("reported offline (reason: IoT Disconnected)"). `onDeviceConnectionStatusChange` already existed — it simply had **no subscriber**, so the signal only ever reached the log. The platform now subscribes *before* streaming starts, so a unit already offline at boot comes up unreachable rather than serving a stale record as live
  - Reads and writes both refuse while unreachable: getters and setters throw `HapStatusError(SERVICE_COMMUNICATION_FAILURE)`, and the error is pushed to the characteristics so the tile updates immediately instead of waiting for HomeKit to poll. A scene can no longer "succeed" against a unit that received nothing. **Local LAN control overrides the cloud verdict** — a unit still reachable over the LAN stays fully controllable whatever the cloud thinks. A stale replay arriving while unreachable is cached but neither published to HomeKit nor mirrored onto a live target, and recovery republishes real state and clears the error
  - Fixed: the post-sweep log said "N device(s) not found on the LAN" even when a unit was plainly on the LAN (its MAC in the ARP cache) but failed to authenticate on a **stale credential** — "not found" implies a network fault that isn't there and points at the wrong fix. Now reported per-device from the MAC→ARP evidence already in hand: "is on the LAN at `<ip>` but its stored local credential didn't authenticate — using cloud (re-pair the unit in the app to refresh its local key)"
  - Fixed: a **single** local-read blip no longer logs a scary "local read failing" + "recovered" pair. Adapters flap `device_authentication_error` for one poll and self-recover on the next (~1-4/hr, pre-existing behavior — measured unchanged before and after the 1.9.0 humidity work); the cloud covers the one-poll gap. A per-serial consecutive-failure counter now surfaces a read only after 3 failures in a row, and once on recovery; single blips stay at debug
  - Fixed: "Set temperature range" was logged on every `profile_update`, a ~15-min heartbeat carrying identical setpoint limits — about 20 identical lines an hour across 5 units, burying real events. Gated on an actual change to the computed range
  - `node:test`: `test/reachability.test.js` (7), `test/local-fail-logging.test.js` (4), plus a range-dedup case in `test/switch-publish.test.js`. **162 tests total green**
  - Code: `platform.ts` (connection subscription, per-device LAN reason, failure counter), `accessory.ts` (`setCloudConnected`/`isReachable`/`assertReachable`, `lastLoggedRange`)
- **1.9.0** - Streaming self-heal, MAC→IP discovery, local humidity (August 2026)
  - Fixed: **the plugin could silently freeze on stale state until a manual restart** (root-caused 2026-08-13, recurred live 2026-08-15). Two independent failure modes, both fixed:
    - **Local-poll wedge:** `local-api.ts` raced the fetch against a timer, but `res.json()` on a stalled response body had no timeout of its own — a unit that returned headers then stalled the body hung the per-serial lock chain and, since the poll loop awaits each read sequentially, the entire local poll loop. Fixed by bounding the *whole* request (headers **and** body read) with an `AbortController`, which tears down the stalled stream and cancels the socket. Proven: the old path hangs ~2.5s, the new one bounds at ~1s
    - **Streaming "zombie":** the health check only asked "is the socket connected?", so a socket that stayed TCP-connected while its `device_update` subscription silently died read as healthy forever — the plugin went deaf to real state changes (device_update dead 13 min while `profile_update`, a ~3.75-min heartbeat, kept flowing). Because the cloud only sends `device_update` on real change, a long silence is ambiguous (idle vs zombie) and no passive check can separate them. `checkStreamingLiveness()` disambiguates with an **active nudge**: after 5 min of silence it requests status from all units; if any answers within a 2.5-min grace the stream is merely idle and it does nothing; if the nudge goes wholly unanswered it's a real zombie and it forces a reconnect (rate-limited). This deliberately replaces an earlier probe that judged health on a 20s nudge window and false-fired on slow adapters. Validated live: silent through a full idle night with zero false fires
  - Added: a coarse **resilience watchdog** as a last-resort backstop — if no accessory has applied an update from *any* source (streaming or local) in 10 minutes, it forces a streaming reconnect + local-poll restart
  - Added: **MAC→IP discovery.** The cloud returns each unit's Wi-Fi adapter MAC in `GET /devices/{serial}/status` (it was long assumed to return neither IP nor MAC). Before sweeping 254 addresses, the plugin now resolves each unit by looking its MAC up in the host's ARP cache (`/proc/net/arp`) — an instant O(1) hit that also re-resolves across DHCP lease changes. Every candidate is still verified with the signed token probe, so a stale ARP entry falls through to the sweep, and the whole fast path degrades to the old sweep on any host without a readable ARP table. Verified live: all controllable units resolved in ~1s (vs the 5–30s sweep)
  - Added: **local humidity + sensor health.** Humidity is the one field the main local status read omits, so under local control it had to keep coming from the (possibly stale/dead) cloud. It's now read over the LAN — an external wireless sensor (`sensors` slots 0–3) or an MHK2 wall control (`indoorHumid`) as fallback — with the source discovered once and cached, so steady-state polling costs one request (has a source) or zero (has neither). A wireless sensor's low battery now warns in the log. Capability-driven: it only lights up on units that actually have a sensor/MHK2. Verified live against real hardware
  - `node:test`: `test/streaming-zombie.test.js` (7), `test/arp-discovery.test.js` (5), `test/local-humidity.test.js` (5), plus the bounded-read regression in `test/local-api.test.js`. **150 tests total green**
  - Code: `kumo-api.ts` (`checkStreamingLiveness`, `getDeviceMac`), `local-api.ts` (AbortController bounding, `parseArpTable`/`candidateIpsByMac`, `getHumidity`), `platform.ts` (resilience watchdog, MAC→IP resolution, humidity poll + low-battery), `accessory.ts` (local humidity)
- **1.8.5** - Mirror a source change that happened while the plugin was down (August 2026)
  - Fixed: **a mirror target silently diverged across an outage.** Mirroring is edge-triggered, and the first source observation after a restart deliberately seeds the baseline *without* pushing — a reboot isn't "someone changed the kitchen", so a manually-adjusted target survives it. That same rule also swallowed a source change that happened while the plugin was **down**, which is a real edge we simply weren't running to observe. The controller had no way to tell the two apart: both look like "first observation, signature X"
  - Observed live 2026-08-05: `hb-service update-node` left a **truncated Node binary** (83,431,529 bytes against an ELF section table pointing at byte 122,075,032 — 38.6 MB missing, `npm` never extracted), Homebridge crash-looped on SIGSEGV every 5s for 41 minutes (458 crashes). The kitchen was switched on at the wall during the outage. When Homebridge came back the mirror seeded its baseline from the already-changed state, and the living room sat `off` against a kitchen at `cool / 24.5` with nothing left to re-sync them — a restart of the child bridge only re-seeded the baseline again
  - Fix: each source's signature is now **persisted across restarts** (`mitsubishi-comfort-mirror-state.json` in the Homebridge storage dir, same write-then-rename pattern as the 1.8.4 credential store). On the first observation after startup the controller compares: **unchanged** since shutdown → seed silently exactly as before, so a manual target change still survives a reboot; **different** → the source moved while we were down, so the targets are re-synced. The store is a cache, never a source of truth — missing or corrupt degrades to precisely the old seed-only behavior and never blocks startup. Signatures carry no secrets (mode, setpoints, fan speed)
  - The mirror contract is otherwise unchanged: still one-way, still edge-triggered, still free between source changes
  - `node:test`: `test/mirror-restart-resync.test.js` (10 cases: the outage case re-syncs, an unchanged source still seeds silently, first-ever run seeds, no-persistence-adapter keeps legacy behavior, a mid-run change persists its signature, the re-sync doesn't re-fire on a second restart, plus store round-trip / missing / corrupt / no-stray-temp-file). **132 tests total green**
  - Code: `src/mirror-store.ts` (new), `mirror.ts:onSourceUpdate` + `MirrorStatePersistence`, `platform.ts:initMirrorStore/mirrorPersistence`
- **1.8.4** - Persist local-control credentials across restarts (July 2026)
  - Fixed: **a working local credential was thrown away on every restart.** The adapter password arrives only when the adapter reports in over the cloud socket, on its own schedule — observed 2.5h after startup (Front bedroom) or never (Living room). Holding it in memory meant each restart re-ran that lottery: on 2026-07-28 two restarts discarded a working credential and put a unit back on cloud control for hours. Confirmed there is no better retrieval (pykumo's flow is a subset of ours, and a fresh-session replay delivered nothing)
  - Fix: new `src/cred-store.ts` persists credentials to one JSON file in the Homebridge storage dir (mode 0600, write-then-rename). `gatherLocalCreds` persists completed push credentials and fills gaps store-first then legacy v2; the LAN sweep persists probe-validated candidates (how v2-sourced creds earn a spot). Trust model unchanged — stored entries are candidates, the signed probe validates them, and the 3-attempt burn budget is **shared** across store and v2 so a stale password can't earn a fresh budget by arriving from a different cache. A corrupt store loads as empty; a missing storage dir disables the store without touching other behavior
  - Code: `src/cred-store.ts` (new), `platform.ts:gatherLocalCreds/persistCred`
- **1.8.3** - Legacy v2 API as a fallback credential source (July 2026)
  - Fixed: **a reachable, healthy unit stranded on cloud control** because its `adapter_update` socket push never arrives (the Living room — hours of nudging, zero deliveries). The credential-capture comment claimed the local password "appears ONLY in the socket push, never via REST" — true of the v3 API but wrong as an absolute
  - Fix: `gatherLocalCreds` now fills credential gaps from the **legacy v2 login** (geo-c, the same source pykumo and Home Assistant use), which returns every adapter's password + cryptoSerial over plain REST. Verified live 2026-07-28: v2-recovered creds signed a successful local status read against the exact unit the push never credentialed. Push creds always win; v2 creds are candidates only (the v2 store can be stale — a rotated password gets `device_authentication_error`), so the LAN sweep's signed probe is the validity gate and a stale password degrades to exactly the prior behavior (cloud). A 3-attempt budget per password stops the minute-cadence retry from re-sweeping forever; the v2 response is cached 6h; secrets are never logged
  - Code: `platform.ts:gatherLocalCreds`, `kumo-api.ts:fetchLegacyCredentials`
